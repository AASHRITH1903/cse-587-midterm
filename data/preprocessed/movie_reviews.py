...
CONFIG_FILE = os.path.join(DATA_DIR, "preprocessed", "movie_reviews_config.json")
EMBED_FILE = os.path.join(DATA_DIR, "preprocessed", "movie_reviews_embeddings.pt")
...
if __name__ == "__main__":
    ...
    for f in movie_reviews.fileids():
        label = f.split("/")[0]
        data_map[label].append(f)

    train_pos, test_pos, train_neg, test_neg = train_test_split(data_map["pos"], data_map["neg"], test_size=0.1, shuffle=True)
    ...
    for doc_id in train_docs:
        all_tokens.extend(tokenize(movie_reviews.raw(doc_id)))
